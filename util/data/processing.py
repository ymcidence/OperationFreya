...
def split_label_unlabel(labels, set_name, labeled_num):
    ...
    pick_per_class = labeled_num // class_num
    labeled_ind = []
    unlabeled_ind = []
    for i in range(class_num):
        ind_list = np.where(labels == i)[0]
        labeled_ind.append(ind_list[:pick_per_class])
        unlabeled_ind.append(ind_list[pick_per_class:])

    return np.concatenate(labeled_ind).astype(np.int32), np.concatenate(unlabeled_ind).astype(np.int32)
...
def construct_label_table(dataset, label_map, labeled=True):
    """Given a label_map file, construct the hash-table it represents.
  Args:
    dataset (str): the name of the dataset.
    label_map (str): the label_map filename.
    labeled (bool): to use the filtered data or not
  Returns:
  ...
  """
    ...
    result_path = os.path.join(path.TRAIN_DIR, dataset, label_map)
    ...
    values = [labeled] * len(keys)
    label_table = tf.lookup.StaticHashTable(
        tf.lookup.KeyValueTensorInitializer(keys=keys, values=values), not labeled)
    return label_table
...
def get_filenames(dataset_name, split):
  ...
  """
    if dataset_name in [
        ...
        filenames = [os.path.join(path.TRAIN_DIR, dataset_name, FILES[split])]
    elif dataset_name == "svhn_extra":
        if split != "train":
            raise ValueError("svhn_extra dataset only has a train split")
        filenames = [os.path.join(path.TRAIN_DIR, "svhn", "extra.tfrecords")]
    else:
